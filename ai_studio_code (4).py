...
import json
import logging
import os  # Yeh line zaroori hai
...
# --- CONFIGURATION ---
# Token ab code se nahi, server ki setting se aayega.
...
async def handle_service_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if service:
        ...
        elif selected_service_name == '🔙 Main Menu':
            return await joined_check(update, context)
        else:
...
async def await_quantity(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if api_response and api_response.get("order"):
            ...
            db["users"][str(user_id)]["orders_value"] = db["users"][str(user_id)].get("orders_value", 0.0) + cost
            save_data(db)
